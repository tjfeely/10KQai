from fetchers import fetch_tenk_report
from langchain.chains.summarize import load_summarize_chain
from langchain.prompts import PromptTemplate
from langchain.schema import Document  # Import Document class
from langchain_community.llms import OpenAI

def summarize_tenk(ticker):
    extracted_text, metadata = fetch_tenk_report(ticker)
    
    # Wrap the extracted text in a Document object
    document = Document(page_content=extracted_text, metadata=metadata)
    
    prompt_template = "Summarize the following 10-K document:\n\n{input_documents}"
...
def ask_about_tenk(ticker, question):
    extracted_text, metadata = fetch_tenk_report(ticker)
